...
{
    public partial class frmRegistroInventarioBodega : Form
    {
        public frmRegistroInventarioBodega()
        {
            InitializeComponent();
            CargarComboArticulo();
        ...
        private void CargarComboArticulo()
        {
            using (GestorArticulo servicio = new GestorArticulo())
                ...
                cbxInventario_ArticuloID.DataSource = servicio.ListarA();
        ...
        private void CargarGridInventario()
        {
            using (GestorInventario servicio = new GestorInventario())
            {
                ...
                dgvInventarioBodega.DataSource = tablenew;
                dgvInventarioBodega.Columns["ID"].Width = 35;
        ...
        private void btnInventario_Insertar_Click(object sender, EventArgs e)
        {
            if (CampoVacio() == 0)
            {
                ...
                {
                    dset = Gestor.ConsultarExistenciaB(Convert.ToInt32(cbxInventario_ArticuloID.SelectedValue));
                ...
                if (dtable.Rows.Count == 0)
                {
                    using (GestorInventario Gestor = new GestorInventario())
                    {
                        Gestor.Insertar(Convert.ToInt32(cbxInventario_ArticuloID.SelectedValue), double.Parse(txbInventario_Cantidad.Text), "B", "A");
                    }
                }
                else
                {
                    using (GestorInventario Gestor = new GestorInventario())
                    {
                        Gestor.Modificar(int.Parse(dtable.Rows[0]["Inventario_id"].ToString()), Convert.ToInt32(cbxInventario_ArticuloID.SelectedValue), (double.Parse(dtable.Rows[0]["Inventario_Cantidad"].ToString()) + double.Parse(txbInventario_Cantidad.Text)), "B", "A");
                    }
                }
                Limpiar();
            }
        ...
        {
            if (CampoVacio() == 0 && id != 0 && id != 0)
            {
                using (GestorInventario Gestor = new GestorInventario())
                {
                    if (chbActivo.Checked == true)
                    {
                        Gestor.Modificar(id, Convert.ToInt32(cbxInventario_ArticuloID.SelectedValue), double.Parse(txbInventario_Cantidad.Text), "B", "A");
                    }
                    else
                    {
                        Gestor.Modificar(id, Convert.ToInt32(cbxInventario_ArticuloID.SelectedValue), double.Parse(txbInventario_Cantidad.Text), "B", "I");
                    }
                }
                Limpiar();
            }
        }
        private void btnInventario_Limpiar_Click_1(object sender, EventArgs e)
        {
            Limpiar();
        }
        ...
        private void dgvInventarioBodega_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                DataSet dset = new DataSet();
                DataTable dtable = new DataTable();
                int numFila = dgvInventarioBodega.CurrentCell.RowIndex;
                id = int.Parse(this.dgvInventarioBodega[0, numFila].Value.ToString());
